# Proxmox VE/PVE NAT Transparent Port Forwarding Manager

# Proxmox VE/PVE NAT 透明端口转发管理脚本

This script is designed for Proxmox VE (PVE) servers with a single public IP address, providing a menu-driven interface for efficiently managing port forwarding rules. Its core design principle is **transparent** forwarding, meaning it forwards traffic to backend VMs or containers without relying on third-party software, effectively **preserving the client's original source IP address**.

本脚本适用于只有一个公网IP的PVE服务器，提供了一个基于菜单的界面，用于在PVE上便捷管理端口转发规则。其设计核心为**透明转发**，即在将流量转发到后端虚拟机/容器时，不依赖第三方软件，能够**保留客户端的原始源 IP 地址**。
...
Save the script content as `pve-nat-port-forward_en.sh`.

2. **Make Executable**
```bash
chmod +x pve-nat-port-forward_en.sh
```

3. **Run Script**
```bash
./pve-nat-port-forward_en.sh
```
...
* **Host Configuration**: Assume your Host's bridge interface (e.g., `vmbr1`) IP is `172.16.1.1`.
* **VM/Container Configuration**: Set the **Gateway** to `172.16.1.1` (The Host's IP).
* *If the VM's gateway is set to your router , forwarding will fail.*
...
* `/etc/port_forward_rules.db`: The rules database.
...
1. **下载/创建脚本**
将脚本内容保存为 `pve-nat-port-forward_cn.sh`。

2. **赋予执行权限**
```bash
chmod +x pve-nat-port-forward_cn.sh
```

3. **运行脚本**
```bash
./pve-nat-port-forward_cn.sh
```
...
* **宿主机配置**：假设宿主机的 `vmbr1` (或您使用的内部网桥) IP 地址为 `172.16.1.1`。
* **虚拟机/容器配置**：在虚拟机的网络设置中，将 **网关 (Gateway)** 设置为 `172.16.1.1`。
* *如果虚拟机网关指向了路由器的 IP ，转发将失败。*
...
* `/etc/port_forward_rules.db`: 规则数据库文件（文本格式，可备份）。
